...
const TeamDetails = () => {
  ...
  const { deleteTeamMembers, isPending: isDeleting } = useDeleteTeamMember();
    ...
    return (
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="bg-white rounded-xl shadow-md p-8 text-center">
          <div className="mx-auto bg-gray-100 text-gray-400 p-4 rounded-full w-16 h-16 flex items-center justify-center mb-4">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
            </svg>
          </div>
          <h3 className="text-xl font-medium text-gray-700 mb-2"> You are not currently in a team</h3>
          <p className="text-gray-500 mb-6">Join or create a team to access team management features</p>
          <div className="flex justify-center gap-4">
            <Button>
          <Link to="/student/team-offers">Browse Team Offers
          </Link>
  ...
  const generalSkills = (teamOfferDetails.general_required_skills || []).map(s => s.name);
  ...
  const progressColor = memberPercentage < 50 ? 'bg-green-500' : 
                       memberPercentage < 80 ? 'bg-blue-500' : 
                       memberPercentage < 100 ? 'bg-yellow-500' : 'bg-red-500';

  return (
    <div className="space-y-6 max-w-6xl mx-auto px-4">
      <section className="bg-white rounded-xl px-6 py-5 shadow-md">
        <SectionTitle
          ...
          <Card className="border border-gray-200 shadow-md">
            ...
            <CardContent className="space-y-6 pt-6">
              <div className="space-y-2">
                ...
                <div className={`p-3 rounded-md text-md ${teamOfferDetails.title ? 'bg-gray-50 border border-gray-200' : 'bg-gray-100 italic'}`}>
              ...
              <div className="space-y-2">
                ...
                <div className={`p-3 rounded-md text-md ${teamOfferDetails.description ? 'bg-gray-50 border border-gray-200' : 'bg-gray-100 italic'}`}>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {allSkills.length > 0 ? allSkills.map((skill, idx) => (
                    <Badge key={`skill-${idx}`} variant="outline" className="text-md bg-blue-50 text-blue-700 border-blue-200">
                      {skill}
                    </Badge>
                  )) : (
                    <div className="text-gray-500 italic">No technologies specified</div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-md font-medium">
                    <span>{currentMembers} member{currentMembers !== 1 ? 's' : ''} joined</span>
                    <span className="text-gray-600">Max {maxMembers} members</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <Progress value={memberPercentage} className={`h-2.5 ${progressColor}`} />
                    <span className="text-sm font-medium text-gray-600">{Math.round(memberPercentage)}%</span>
                  </div>
                </div>
              </div>
            </CardContent>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-semibold text-lg text-gray-800">
                    {teamOfferDetails.leader?.user?.firstName} {teamOfferDetails.leader?.user?.lastName}
                  </h4>
            ...
            <CardHeader className="bg-gray-50 border-b">
              <div className="flex justify-between items-center">
                ...
                <Badge variant="outline" className="bg-white text-gray-600">
                  {currentMembers - 1} member{currentMembers - 1 !== 1 ? 's' : ''}
                </Badge>
              </div>
            </CardHeader>
            <CardContent className="pt-6">
              {teamOfferDetails.TeamMembers?.length > 1 ? (
                            ...
                            <DialogContent className="sm:max-w-md rounded-lg">
                              <DialogHeader className="space-y-4">
                                ...
                                <DialogDescription className="text-center text-gray-600">
                                  This will permanently remove {member.student.user.firstName} from the team.
                                  <br />
                              ...
                              <DialogFooter className="gap-3 sm:gap-0">
                                ...
                                >
                                  {isDeleting ? (
                                    <ButtonWithSpinner disabled={isDeleting} />
                                  ) : (
                                    "Confirm Removal"
                                  )}
                                </Button>
                ...
                <div className="text-center py-8">
                  ...
                  <h4 className="text-gray-600 font-medium">No team members yet</h4>
                  <p className="text-gray-500 text-sm mt-1">Only the team leader is currently part of this team</p>
                </div>
              )}
            </CardContent>
          </Card>
